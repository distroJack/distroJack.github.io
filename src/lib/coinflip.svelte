...
<style>
  ...
  @keyframes flip {
    ...
    25%  { transform: translateY(-200px) rotateY(720deg); }
    50%  { transform: translateY(0) rotateY(1440deg); }
    75%  { transform: translateY(-100px) rotateY(2160deg); }
...
<div
  class="coin {isFlipping ? 'flip' : ''}"
  style="background-image: url({coinSide});"
></div>

<button class="button" on:click={flipCoin} disabled={isFlipping}>
  Flip the Coin
</button>
